...
function extractProfile(
  ...
  // ── Open questions (last Jade message) ──
...
// ─── Public function (used by jade/chatwoot/route.ts) ────────────────────────
...
export async function fetchClientMemory(conversationId: number | null): Promise<MemoryResult> {
  if (!conversationId) return { profile: { ...EMPTY_PROFILE }, messages: [], contactId: null }

  try {
    const { messages: rawMessages, contactId } = await fetchConversationMessages(conversationId)
    const messages = rawMessages.slice(-30) // sliding window: last 30
    ...
    return { profile: { ...EMPTY_PROFILE }, messages: [], contactId: null }
  }
}
