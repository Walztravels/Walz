...
function StepThree({ tour, date, groupSize, addons, details, onBack, onSuccess }: {
  ...
  function handlePay() {
    ...
    if (gateway === 'stripe') { handleStripePay() } else { handleFlutterwavePay() }
  }
        ...
        <div className="lg:sticky lg:top-24 space-y-3">
          <div className="bg-[#0B1F3A] rounded-2xl p-5 text-white shadow-lg">
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              ...
              >
                <CreditCard className={`w-5 h-5 ${gateway === 'stripe' ? 'text-[#C9A84C]' : 'text-white/60'}`} />
                <span className={`text-xs font-bold leading-none ${gateway === 'stripe' ? 'text-white' : 'text-white/60'}`}>
                  Card / Bank
                </span>
                <span className={`text-[10px] leading-none ${gateway === 'stripe' ? 'text-[#C9A84C]' : 'text-white/40'}`}>
              ...
              <button
                ...
                className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 transition-all
                  ${gateway === 'flutterwave'
                    ...
                    : 'border-white/10 hover:border-white/25 bg-white/5'}`}
              >
                <Smartphone className={`w-5 h-5 ${gateway === 'flutterwave' ? 'text-[#C9A84C]' : 'text-white/60'}`} />
                <span className={`text-xs font-bold leading-none ${gateway === 'flutterwave' ? 'text-white' : 'text-white/60'}`}>
                  Mobile / Other
                </span>
                <span className={`text-[10px] leading-none ${gateway === 'flutterwave' ? 'text-[#C9A84C]' : 'text-white/40'}`}>
            ...
            {/* Gateway description */}
            <p className="text-white/40 text-[11px] mb-4 text-center">
              {gateway === 'stripe'
        ...
        <div className="flex gap-2 mb-2">
          <button
            ...
            className={`flex-1 py-1.5 rounded-lg text-xs font-bold border-2 transition-all
              ${gateway === 'stripe' ? 'border-[#C9A84C] bg-[#C9A84C]/10 text-[#0B1F3A]' : 'border-gray-200 text-gray-400'}`}
          >
            💳 Card (Stripe)
          </button>
          <button
            ...
            onClick={() => setGateway('flutterwave')}
            className={`flex-1 py-1.5 rounded-lg text-xs font-bold border-2 transition-all
              ${gateway === 'flutterwave' ? 'border-[#C9A84C] bg-[#C9A84C]/10 text-[#0B1F3A]' : 'border-gray-200 text-gray-400'}`}
          >
            📱 Mobile / Other
          </button>
        </div>
