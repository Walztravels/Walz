        ...
        <div className="max-w-2xl mx-auto">
          ...
          {itinerary && (
            <>
              ...
              <div className="mt-2">
                <div className="inline-flex items-center gap-1.5 bg-amber-500/15 border border-amber-500/30 px-3 py-1.5 rounded-full">
                  <span className="text-amber-400 text-sm">✨</span>
                  <span className="text-amber-400 text-xs font-semibold">Created by Jade · Walz Travels</span>
                </div>
              </div>
            </>
          )}
            ...
            {visaMatrix.length > 0 && (
              <div>
                ...
                <div className="space-y-3">
                  {visaMatrix.map((v, i) => {
                    const borderClass = v.missingNationality
                      ? 'bg-white/5 border-white/10'
                      : v.rule.status === 'required'   ? 'bg-amber-500/10  border-amber-500/30'
                      : v.rule.status === 'on_arrival' ? 'bg-blue-500/10   border-blue-500/30'
                      : v.rule.status === 'evisa'      ? 'bg-purple-500/10 border-purple-500/30'
                      :                                  'bg-emerald-500/10 border-emerald-500/30'

                    if (v.missingNationality) {
                      return (
                        <div key={i} className={`rounded-2xl p-4 border ${borderClass}`}>
                          <p className="text-white font-semibold text-sm">{v.memberName}</p>
                          {v.flyingFrom && v.flyingFrom !== 'Unknown' && (
                            <p className="text-white/30 text-xs mt-0.5">Flying from {v.flyingFrom}</p>
                          )}
                          <p className="text-white/40 text-xs mt-3">
                            🌍 Nationality not provided — ask them to update their travel profile
                          </p>
                        </div>
                      )
                    }

                    const statusEmoji = v.rule.status === 'required'   ? '⚠️'
                                      : v.rule.status === 'on_arrival' ? '🔵'
                                      : v.rule.status === 'evisa'      ? '💻'
                                      :                                   '✅'

                    const statusLabel = v.rule.status === 'required'   ? 'VISA REQUIRED'
                                      : v.rule.status === 'on_arrival' ? 'VISA ON ARRIVAL'
                                      : v.rule.status === 'evisa'      ? 'eVISA REQUIRED'
                                      :                                   'NO VISA REQUIRED'

                    const statusColor = v.rule.status === 'required'   ? 'text-amber-400'
                                      : v.rule.status === 'on_arrival' ? 'text-blue-400'
                                      : v.rule.status === 'evisa'      ? 'text-purple-400'
                                      :                                   'text-emerald-400'
                      ...
                      <div key={i} className={`rounded-2xl p-4 border ${borderClass}`}>
                        <div className="mb-3">
                          ...
                          <p className="text-white/40 text-[11px] mt-0.5">
                            {v.passport} passport
                            {v.flyingFrom && v.flyingFrom !== 'Unknown' ? ` · Flying from ${v.flyingFrom}` : ''}
                          </p>
                        </div>
                        ...
                        {v.rule.visaType && (
                          <p className="text-white/60 text-xs mb-2">{v.rule.visaType}</p>
                        )}

                        {v.rule.status === 'free' ? (
                          <p className="text-white/40 text-xs">{v.rule.notes}</p>
                        ) : (
                          <>
                              ...
                              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-2">
                                {v.rule.cost && (
                                  <span className="text-white/70 text-xs">
                                    <span className="text-white/30 text-[10px] uppercase tracking-wide">Cost </span>
                                    {v.rule.cost}
                                  </span>
                                )}
                                {v.rule.processingTime && (
                                  <span className="text-white/70 text-xs">
                                    <span className="text-white/30 text-[10px] uppercase tracking-wide">Processing </span>
                                    {v.rule.processingTime}
                                  </span>
                                )}
                              </div>
