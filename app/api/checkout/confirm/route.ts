...
// ── Cart sessions (item_count in metadata) ────────────────────────────────────
// For cart-based checkouts, the Stripe webhook is the authoritative trigger for
// supplier booking and email. This endpoint only reads the resulting status —
...
async function handleCartSession(sessionId: string) {
  ...
  // All ActivityBookings for this Stripe session (one per cart activity item)
  ...
  const cartStatus = bookings.length
  ...
  return {
    ...
    supplierReference: (bookings.find(b => b.status === 'CONFIRMED') ?? bookings[0])?.supplierReference ?? null,
    status:            cartStatus,
    customerEmail:     bookings[0]?.clientEmail ?? session.customer_details?.email ?? '',
...
// ── Non-cart / legacy sessions ────────────────────────────────────────────────
// Pre-existing behavior: create a generic Booking record and email the voucher.
// Used by one-off checkout sessions that do NOT have item_count metadata.

async function handleLegacySession(sessionId: string | undefined, gateway: string, txRef: string | undefined) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    // Detect cart sessions by peeking at Stripe metadata
    if (gateway !== 'flutterwave' && sessionId) {
      const session = await stripe.checkout.sessions.retrieve(sessionId)

      if (session.metadata?.item_count) {
        // New cart path: webhook handles everything; we just return the recorded status
        const result = await handleCartSession(sessionId)
        console.log('[Cart confirm — cart path]', result)
        return NextResponse.json(result)
      }
    }

    // Legacy / non-cart path
    const result = await handleLegacySession(sessionId, gateway, txRef)
    console.log('[Cart confirm — legacy path]', { ref: result.bookingReference, gateway, txRef })
    return NextResponse.json(result)

  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[Cart confirm]', msg)
    return NextResponse.json({ bookingReference: null, status: 'ERROR', error: msg })
  }
}
