 ...
 * 1. Confirms payment with Battleface
 * 2. Updates order status to 'approved'
 * 3. Emails policy document to client
 */
export async function POST(req: NextRequest) {
  ...
  // ── Confirm payment with Battleface ────────────────────────────────────────
  ...
  try {
    ...
    if (bfRes.ok) {
      ...
      // Continue anyway — Stripe payment succeeded; mark approved and email
    }
  } catch (err) {
    console.warn('[POST /api/insurance/payment] bf confirmation error:', err)
    // Continue — don't block policy delivery if battleface is unreachable
  }
  ...
  // ── Send policy email ──────────────────────────────────────────────────────
  const pt = dbOrder.primary_traveller as {
    first_name?: string
    last_name?:  string
    email?:      string
  } | null
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:32px 24px">
          <div style="background:#0B1F3A;border-radius:12px;padding:28px;margin-bottom:28px">
            ...
            <p style="color:rgba(255,255,255,0.6);margin:0;font-size:14px">Walz Travel Shield — Powered by Battleface</p>
          </div>
