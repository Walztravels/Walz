...
function CopyFixButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="text-xs text-green-500 hover:text-green-300 transition-colors ml-auto flex-shrink-0"
    >
...
export default function AuditDetailPage() {
  ...
  type GscRow = { pageId: string; url: string; gsc: { clicks: number; impressions: number; ctr: number; position: number } | null }
  ...
  const gscByPageId = new Map(gscData.map(r => [r.pageId, r.gsc]))

  async function loadGsc() {
    setEnriching(true); setEnrichMsg(null)
    try {
      const res = await fetch('/api/admin/orbit/integrations/gsc/enrich', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ auditId: id }),
      })
      const data = await res.json() as { enriched?: number; data?: GscRow[]; error?: string }
      if (!res.ok) throw new Error(data.error ?? 'GSC enrich failed')
      setGscData(data.data ?? [])
      setEnrichMsg(`GSC data loaded for ${data.enriched} pages (last 28 days)`)
    } catch (e) {
      setEnrichMsg(e instanceof Error ? e.message : 'Failed to load GSC data')
    } finally {
      setEnriching(false)
    }
  }
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {audit.status === 'complete' && (
              <button
                onClick={loadGsc}
                disabled={enriching}
                className="text-xs bg-gray-800 hover:bg-gray-700 disabled:opacity-50 text-gray-300 px-3 py-1 rounded-lg transition-colors ml-auto"
              >
                {enriching ? 'Loading GSC…' : gscData.length > 0 ? 'Refresh GSC data' : 'Load GSC data'}
              </button>
            )}
          </div>
          {enrichMsg && (
            <p className={`text-xs mt-1 ${enrichMsg.includes('Failed') || enrichMsg.includes('error') ? 'text-red-400' : 'text-green-400'}`}>
              {enrichMsg}
            </p>
          )}
            ...
            <div className="space-y-3">
              {visibleIssues.map((issue) => (
                <div key={issue.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-2">
                  ...
                  {issue.fixPreview && (
                    <div className="bg-green-950/30 border border-green-900 rounded">
                      <div className="flex items-center gap-2 px-3 py-1.5 border-b border-green-900/50">
                        <span className="text-xs text-green-500 font-medium">Suggested fix</span>
                        <CopyFixButton text={issue.fixPreview} />
                      </div>
                      <pre className="text-xs text-green-300 px-3 py-2 whitespace-pre-wrap overflow-x-auto">{issue.fixPreview}</pre>
                    </div>
                  )}
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-800 text-left text-gray-500 uppercase">
                    ...
                    {gscData.length > 0 && <>
                      <th className="px-4 py-3 font-medium text-blue-400">Clicks</th>
                      <th className="px-4 py-3 font-medium text-blue-400">Impr.</th>
                      <th className="px-4 py-3 font-medium text-blue-400">Pos.</th>
                    </>}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {audit.pages.map((p) => {
                    ...
                    const gsc = gscByPageId.get(p.id)
                    return (
                      <tr key={p.id} className="hover:bg-gray-800/40 transition-colors">
                        ...
                        {gscData.length > 0 && <>
                          <td className="px-4 py-3 tabular-nums text-blue-300">{gsc ? gsc.clicks.toLocaleString() : <span className="text-gray-700">—</span>}</td>
                          <td className="px-4 py-3 tabular-nums text-blue-300">{gsc ? gsc.impressions.toLocaleString() : <span className="text-gray-700">—</span>}</td>
                          <td className="px-4 py-3 tabular-nums text-blue-300">{gsc ? gsc.position.toFixed(1) : <span className="text-gray-700">—</span>}</td>
                        </>}
                      </tr>
                    )
                  })}
