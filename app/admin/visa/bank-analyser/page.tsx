            ...
            <div className="p-8 space-y-6">
              ...
              {(analysis.monthlyBreakdown?.length ?? 0) > 0 && (
                <div className="print-section space-y-4">
                  <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Month-by-Month Breakdown</h3>
                  {analysis.monthlyBreakdown!.map((m, mi) => (
                    <div key={mi} className="border border-gray-200 rounded-xl overflow-hidden">
                      {/* Month header */}
                      <div className="bg-[#0B1F3A]/5 px-4 py-3 flex items-center justify-between flex-wrap gap-2">
                        <p className="font-semibold text-[#0B1F3A] text-sm">{m.month}</p>
                        <div className="flex gap-4 text-xs text-gray-500">
                          <span>Open: <b className="text-[#0B1F3A]">{analysis.currency} {m.openingBalance?.toLocaleString()}</b></span>
                          <span>Close: <b className="text-[#0B1F3A]">{analysis.currency} {m.closingBalance?.toLocaleString()}</b></span>
                          <span className="text-green-600">+{analysis.currency} {m.totalCredits?.toLocaleString()}</span>
                          <span className="text-red-500">−{analysis.currency} {m.totalDebits?.toLocaleString()}</span>
                        </div>
                      </div>
                      {/* Transactions table */}
                          ...
                          <table className="w-full text-xs">
                            <thead>
                              <tr className="border-b border-gray-100 bg-gray-50">
                                <th className="text-left px-3 py-2 text-gray-400 font-medium w-24">Date</th>
                                <th className="text-left px-3 py-2 text-gray-400 font-medium">Description</th>
                                <th className="text-right px-3 py-2 text-gray-400 font-medium w-28">Credit</th>
                                <th className="text-right px-3 py-2 text-gray-400 font-medium w-28">Debit</th>
                                <th className="text-right px-3 py-2 text-gray-400 font-medium w-28">Balance</th>
                              </tr>
                            </thead>
                            <tbody>
                              {m.transactions.map((tx, ti) => (
                                <tr key={ti} className={`border-b border-gray-50 ${tx.flag ? 'bg-red-50' : ''}`}>
                                  <td className="px-3 py-1.5 text-gray-500 whitespace-nowrap">{tx.date}</td>
                                  <td className="px-3 py-1.5 text-gray-700">
                                    {tx.description}
                                    {tx.flag && <span className="ml-2 text-red-500 text-xs">⚠ {tx.flag}</span>}
                                  </td>
                                  <td className="px-3 py-1.5 text-right text-green-700 font-medium">
                                    {tx.type === 'credit' ? `${analysis.currency} ${tx.amount?.toLocaleString()}` : ''}
                                  </td>
                                  <td className="px-3 py-1.5 text-right text-red-600 font-medium">
                                    {tx.type === 'debit' ? `${analysis.currency} ${tx.amount?.toLocaleString()}` : ''}
                                  </td>
                                  <td className="px-3 py-1.5 text-right text-gray-700 font-medium">
                                    {tx.runningBalance != null ? `${analysis.currency} ${tx.runningBalance?.toLocaleString()}` : '—'}
                                  </td>
                                </tr>
                              ))}
                            </tbody>
              ...
              {(analysis.suspiciousTransactions?.length ?? 0) > 0 && (
                <div className="print-section">
                  <h3 className="text-xs font-semibold text-red-500 uppercase tracking-widest mb-3">🚩 Flagged Transactions</h3>
                  <div className="space-y-2">
                    {analysis.suspiciousTransactions!.map((t, i) => (
                      <div key={i} className="flex items-start gap-3 bg-red-50 rounded-xl p-3 border border-red-100 text-sm">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-red-600 text-xs mt-0.5 font-medium">{t.reason}</p>
                        </div>
              ...
              {/* Balance dips */}
              {(analysis.balanceDipsBelow?.length ?? 0) > 0 && (
                <div className="print-section">
                  <h3 className="text-xs font-semibold text-amber-600 uppercase tracking-widest mb-3">⬇ Balance Below Threshold</h3>
                  <div className="space-y-1.5">
                    {analysis.balanceDipsBelow!.map((d, i) => (
                      <div key={i} className="flex items-center gap-3 bg-amber-50 rounded-xl p-3 border border-amber-100 text-sm">
                        <span className="text-amber-500">📉</span>
                        <span className="flex-1 text-gray-700">{d.date} — balance dropped to</span>
                        <span className="font-semibold text-red-600 flex-shrink-0">{analysis.currency} {d.balance?.toLocaleString()}</span>
                        <span className="text-gray-400 text-xs">(min: {analysis.embassyCurrency} {d.threshold?.toLocaleString()})</span>
                      </div>
                    ))}
                  </div>
                </div>
