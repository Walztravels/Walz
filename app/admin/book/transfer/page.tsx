  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="font-bold text-[#0B1F3A] text-xl">Book a Transfer</h1>
          <p className="text-gray-400 text-xs">Hotelbeds · Airport transfers · Private vehicles</p>
        </div>
      </div>
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input value={pickup} onChange={e => setPickup(e.target.value)}
            placeholder="Pickup location (airport, hotel, address)" required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" value={adults} onChange={e => setAdults(Number(e.target.value))} min={1} max={16}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#C9A84C] mt-1" />
            </div>
            <div>
          ...
          <button type="submit"
            className="w-full flex items-center justify-center gap-2 bg-purple-600 text-white font-bold py-3 rounded-xl hover:bg-purple-700 transition-colors">
