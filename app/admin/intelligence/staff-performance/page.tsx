...
export default function StaffPerformancePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    setLoadError('')
    const result = await fetchRecords<StaffMetric>('/api/admin/intelligence/staff-performance', 'metrics')
    if (result.ok) setMetrics(result.records)
    else { setMetrics([]); setLoadError(result.error) }
    setLoading(false)
  }, [])
