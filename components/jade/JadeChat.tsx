...
import {
  ...
  Phone, Mail, Loader2, RotateCcw, UserCheck, Shield,
} from 'lucide-react'
...
// ─── localStorage helpers (persistent across sessions) ────────────────────────
...
export function JadeChat() {
  ...
  const sendMessage = useCallback(async (text?: string) => {
    ...
    try {
      // Persist session & conversation IDs in localStorage for cross-session memory
      ...
      // Persist conversationId for memory continuity
      if (data.conversationId) {
        lsSet('jade_conv_id', String(data.conversationId))
      }

      // Agent has taken over → Jade goes silent
      if (data.silenced) {
        setAgentActive(true)
        setMessages(prev => [...prev, {
          id:        (Date.now() + 1).toString(),
          role:      'assistant',
          content:   '👤 One of our consultants is now handling your conversation. They\'ll be with you shortly — no need to repeat yourself.',
          timestamp: new Date(),
          isAgent:   true,
        }])
        return
      ...
      // Jade resumed after agent went quiet
  ...
  const handleReset = () => {
    ...
    lsRemove('jade_sid'); lsRemove('jade_conv_id')
    handleOpen()
  }
...
// Re-export as JadeChatWidget for backward-compatibility with existing imports
