...
/**
 * TicketPreview — live HTML preview of a Walz Travels ticket.
 * Mirrors the PDF design using Tailwind + inline styles.
 */
  ...
  return (
    <>
      ...
      <Row>
        ...
        <FieldPair label="PNR"            value={d.pnr as string} />
      </Row>
      {pax.length > 0 && (
        <>
          ...
          {pax.map((p, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-3 rounded-lg p-2 mb-1.5" style={{ backgroundColor: '#F3F4F6' }}>
                <span className="text-sm font-bold w-5 text-center" style={{ color: GOLD }}>{i + 1}</span>
                <div>
                  <div className="text-xs font-bold" style={{ color: NAVY }}>{px.name}</div>
                  <div className="text-[10px] text-gray-400">Passport: {px.passport} · Seat: {px.seat}</div>
                </div>
              </div>
            )
          })}
