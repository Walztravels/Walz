...
import {
  ...
  CheckSquare, Users, LogOut, Globe, ChevronRight, UserCircle, MessageCircle,
} from 'lucide-react'
import { cn } from '@/lib/utils'

const NAV = [
  { href: '/portal/dashboard',   label: 'My Applications', icon: LayoutDashboard },
    ...
    <aside className="flex flex-col w-60 min-h-screen bg-[#0B1F3A] text-white flex-shrink-0">
      ...
      <div className="px-5 py-5 border-b border-white/10">
        <Link href="/" className="flex flex-col items-start">
          <Image src="/walz-logo.png" alt="Walz Travels" width={110} height={40} className="w-[110px] h-auto object-contain" />
      ...
      <div className="px-3 py-4 border-t border-white/10 space-y-1">
        <Link href="/" target="_blank" className="flex items-center gap-3 px-3 py-2 rounded-lg text-xs text-white/50 hover:text-white/80 hover:bg-white/10 transition-all">
          <Globe className="w-4 h-4" />
          Back to Website
        </Link>
        <a href="https://wa.me/447398753797" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2 rounded-lg text-xs text-green-400/80 hover:text-green-300 hover:bg-white/10 transition-all">
